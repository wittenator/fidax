...
# - jax-fid by Matthias Wright (https://github.com/matthias-wright/jax-fid)
#   Copyright 2021 Matthias Wright, Licensed under Apache License 2.0
#
# The original Inception V3 implementation is from:
...
class InceptionV3(nn.Module):
    ...
    def __call__(self, x, train=True, rng=jax.random.PRNGKey(0)):
        ...
        x = BasicConv2d(
            out_channels=32, kernel_size=(3, 3), params_dict=get(self.params_dict, "Conv2d_2a_3x3"), dtype=self.dtype
        )(x, train)
        x = BasicConv2d(
            out_channels=64,
            kernel_size=(3, 3),
        ...
        x = BasicConv2d(
            out_channels=80, kernel_size=(1, 1), params_dict=get(self.params_dict, "Conv2d_3b_1x1"), dtype=self.dtype
        )(x, train)
        x = BasicConv2d(
            out_channels=192, kernel_size=(3, 3), params_dict=get(self.params_dict, "Conv2d_4a_3x3"), dtype=self.dtype
        )(x, train)
        x = nn.max_pool(x, window_shape=(3, 3), strides=(2, 2))
        x = InceptionA(pool_features=32, params_dict=get(self.params_dict, "Mixed_5b"), dtype=self.dtype)(x, train)
        x = InceptionA(pool_features=64, params_dict=get(self.params_dict, "Mixed_5c"), dtype=self.dtype)(x, train)
        x = InceptionA(pool_features=64, params_dict=get(self.params_dict, "Mixed_5d"), dtype=self.dtype)(x, train)
        x = InceptionB(params_dict=get(self.params_dict, "Mixed_6a"), dtype=self.dtype)(x, train)
        x = InceptionC(channels_7x7=128, params_dict=get(self.params_dict, "Mixed_6b"), dtype=self.dtype)(x, train)
        x = InceptionC(channels_7x7=160, params_dict=get(self.params_dict, "Mixed_6c"), dtype=self.dtype)(x, train)
        x = InceptionC(channels_7x7=160, params_dict=get(self.params_dict, "Mixed_6d"), dtype=self.dtype)(x, train)
        x = InceptionC(channels_7x7=192, params_dict=get(self.params_dict, "Mixed_6e"), dtype=self.dtype)(x, train)
        aux = None
        if self.aux_logits and train:
            aux = InceptionAux(
                num_classes=self.num_classes_, params_dict=get(self.params_dict, "AuxLogits"), dtype=self.dtype
            )(x, train)
        x = InceptionD(params_dict=get(self.params_dict, "Mixed_7a"), dtype=self.dtype)(x, train)
        x = InceptionE(avg_pool, params_dict=get(self.params_dict, "Mixed_7b"), dtype=self.dtype)(x, train)
        ...
        x = InceptionE(nn.max_pool, params_dict=get(self.params_dict, "Mixed_7c"), dtype=self.dtype)(x, train)
        ...
        x = Dense(features=self.num_classes_, params_dict=get(self.params_dict, "fc"), dtype=self.dtype)(x)
        if self.aux_logits:
            return x, aux
        return x
...
class Dense(nn.Module):
    ...
    def __call__(self, x):
        x = nn.Dense(
            ...
            bias_init=self.bias_init if self.params_dict is None else lambda *_: jnp.array(self.params_dict["bias"]),
        )(x)
        return x
...
class InceptionA(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        branch5x5 = BasicConv2d(
            out_channels=48, kernel_size=(1, 1), params_dict=get(self.params_dict, "branch5x5_1"), dtype=self.dtype
        )(x, train)
        branch5x5 = BasicConv2d(
            out_channels=64,
            kernel_size=(5, 5),
        ...
        branch3x3dbl = BasicConv2d(
            out_channels=64, kernel_size=(1, 1), params_dict=get(self.params_dict, "branch3x3dbl_1"), dtype=self.dtype
        )(x, train)
        branch3x3dbl = BasicConv2d(
            out_channels=96,
            kernel_size=(3, 3),
...
class InceptionC(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        branch_pool = avg_pool(x, window_shape=(3, 3), strides=(1, 1), padding=((1, 1), (1, 1)))
        branch_pool = BasicConv2d(
            out_channels=192, kernel_size=(1, 1), params_dict=get(self.params_dict, "branch_pool"), dtype=self.dtype
        )(branch_pool, train)
...
class InceptionD(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        branch7x7x3 = BasicConv2d(
            out_channels=192, kernel_size=(1, 1), params_dict=get(self.params_dict, "branch7x7x3_1"), dtype=self.dtype
        )(x, train)
        branch7x7x3 = BasicConv2d(
            out_channels=192,
            kernel_size=(1, 7),
...
class InceptionE(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        branch3x3 = BasicConv2d(
            out_channels=384, kernel_size=(1, 1), params_dict=get(self.params_dict, "branch3x3_1"), dtype=self.dtype
        )(x, train)
        ...
        branch3x3dbl = BasicConv2d(
            out_channels=448, kernel_size=(1, 1), params_dict=get(self.params_dict, "branch3x3dbl_1"), dtype=self.dtype
        )(x, train)
        branch3x3dbl = BasicConv2d(
            out_channels=384,
            kernel_size=(3, 3),
...
class InceptionAux(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = BasicConv2d(
            out_channels=768, kernel_size=(5, 5), params_dict=get(self.params_dict, "conv1"), dtype=self.dtype
        )(x, train)
        x = jnp.mean(x, axis=(1, 2))
        x = jnp.reshape(x, newshape=(x.shape[0], -1))
        x = Dense(features=self.num_classes, params_dict=get(self.params_dict, "fc"), dtype=self.dtype)(x)
        return x
